...
begin
  if Pos(Signature(e), sRecordsToSkip) <> 0 then
    Exit;
    //if Pos('SCORE', EditorID(e)) = 0 then
    //Exit;
    if Pos('_Weekly_', EditorID(e)) = 0 then
     Exit;
    if Pos('Epic', EditorID(e)) <> 0 then
     Exit;
   if Pos('CUT_', EditorID(e)) <> 0 then
     Exit;
   if Pos('TEST', EditorID(e)) <> 0 then
     Exit;
...
begin
  if slExport.Count <> 0 then begin
    ...
    try
      ...
      dlgSave.FileName := 'weekly_challengesP.txt';
